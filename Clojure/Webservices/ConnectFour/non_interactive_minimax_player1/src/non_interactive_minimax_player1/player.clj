...
(defn init-game [req]
  (let [result (non-interactive-minimax-player/player1 {:data ["init-game"]})]
...
(defn get-first-move [req]
  (if @game-initialised
    (let [
           result (non-interactive-minimax-player/player1 {:data ["get-first-move"]})
         ]
         {
           :status 200
           :headers {"Content-Type" "application/json"}
  ...
  (if @game-initialised
    (let [
	   move-value (:move-value (:params req))
           result (non-interactive-minimax-player/player1 {:data ["get-next-move" move-value]})
         ]
         {
           :status 200
           :headers {"Content-Type" "application/json"}
...
(defn notify-move [req]
  (if @game-initialised
    (let [
	   move-value (:move-value (:params req))
           result (non-interactive-minimax-player/player1 {:data ["notify-move" move-value]})
         ]
         {
           :status 200
           :headers {"Content-Type" "application/json"}
