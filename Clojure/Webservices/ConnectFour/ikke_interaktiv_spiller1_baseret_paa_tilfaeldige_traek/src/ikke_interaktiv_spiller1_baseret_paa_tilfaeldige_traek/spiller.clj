(ns ikke-interaktiv-spiller1-baseret-paa-tilfaeldige-traek.spiller
  (:require [org.httpkit.server :as server]
	    ...
	    [games.connect-four.ikke-interaktiv-spiller-baseret-paa-tilfaeldige-traek :as ikke-interaktiv-spiller-baseret-paa-tilfaeldige-traek]
  )
  (:gen-class)
)
...
(defn initialiser-spil [req]
  (let [resultat (ikke-interaktiv-spiller-baseret-paa-tilfaeldige-traek/spiller1 {:data ["initialiserSpil"]})]
...
(defn hent-foerste-traek [req]
  (if @spillet-er-initialiseret?
    (let [
           resultat (ikke-interaktiv-spiller-baseret-paa-tilfaeldige-traek/spiller1 {:data ["hentFoersteTraek"]})
         ]
         {
           :status 200
           :headers {"Content-Type" "application/json"}
  ...
  (if @spillet-er-initialiseret?
    (let [
	   traekvaerdi (:traekvaerdi (:params req))
           resultat (ikke-interaktiv-spiller-baseret-paa-tilfaeldige-traek/spiller1 {:data ["hentNaesteTraek" traekvaerdi]})
         ]
         {
           :status 200
           :headers {"Content-Type" "application/json"}
...
(defn meddel-traek [req]
  (if @spillet-er-initialiseret?
    (let [
	   traekvaerdi (:traekvaerdi (:params req))
           resultat (ikke-interaktiv-spiller-baseret-paa-tilfaeldige-traek/spiller1 {:data ["meddelTraek" traekvaerdi]})
         ]
         {
           :status 200
           :headers {"Content-Type" "application/json"}
...
(defroutes app-routes
  (GET "/" [] "Dette er en ikke-interaktiv spiller1 til spillet \"Fire-paa-stribe\".")
