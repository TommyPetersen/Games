(ns ikke-interaktiv-spiller2-baseret-paa-tilfaeldige-traek.spiller
...
(defn initialiser-spil [req]
  (let [resultat (ikke-interaktiv-spiller-baseret-paa-tilfaeldige-traek/spiller2 {:data ["initialiserSpil"]})]
  ...
  (if @spillet-er-initialiseret?
    (let [
	   traekvaerdi (:traekvaerdi (:params req))
           resultat (ikke-interaktiv-spiller-baseret-paa-tilfaeldige-traek/spiller2 {:data ["hentNaesteTraek" traekvaerdi]})
         ]
         {
           :status 200
           :headers {"Content-Type" "application/json"}
...
(defn meddel-traek [req]
  (if @spillet-er-initialiseret?
    (let [
	   traekvaerdi (:traekvaerdi (:params req))
           resultat (ikke-interaktiv-spiller-baseret-paa-tilfaeldige-traek/spiller2 {:data ["meddelTraek" traekvaerdi]})
         ]
         {
           :status 200
           :headers {"Content-Type" "application/json"}
...
(defroutes app-routes
  (GET "/" [] "Dette er en ikke-interaktiv spiller2 til spillet \"Fire-paa-stribe\".")
