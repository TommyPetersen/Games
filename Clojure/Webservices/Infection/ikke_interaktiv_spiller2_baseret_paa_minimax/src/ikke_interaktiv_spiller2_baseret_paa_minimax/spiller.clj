(ns ikke-interaktiv-spiller2-baseret-paa-minimax.spiller
  (:require [org.httpkit.server :as server]
	    ...
	    [games.infection.ikke-interaktiv-spiller-baseret-paa-minimax :as ikke-interaktiv-spiller-baseret-paa-minimax]
  )
  (:gen-class)
)
...
(defn initialiser-spil [req]
  (let [resultat (ikke-interaktiv-spiller-baseret-paa-minimax/spiller2 {:data ["initialiserSpil"]})]
  ...
  (if @spillet-er-initialiseret?
    (let [
	   traekvaerdi (:traekvaerdi (:params req))
           resultat (ikke-interaktiv-spiller-baseret-paa-minimax/spiller2 {:data ["hentNaesteTraek" traekvaerdi]})
         ]
         {
           :status 200
           :headers {"Content-Type" "application/json"}
...
(defn meddel-traek [req]
  (if @spillet-er-initialiseret?
    (let [
	   traekvaerdi (:traekvaerdi (:params req))
           resultat (ikke-interaktiv-spiller-baseret-paa-minimax/spiller2 {:data ["meddelTraek" traekvaerdi]})
         ]
         {
           :status 200
           :headers {"Content-Type" "application/json"}
