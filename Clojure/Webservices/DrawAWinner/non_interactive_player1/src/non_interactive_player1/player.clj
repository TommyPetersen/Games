(ns non-interactive-player1.player
  (:require [org.httpkit.server :as server]
	    ...
	    [games.draw-a-winner.non-interactive-player1 :as non-interactive-player1]
  )
  (:gen-class)
)
...
(defn init-game [req]
  (let [result (non-interactive-player1/player1 {:data ["init-game"]})]
...
(defn get-first-move [req]
  ...
  (if @game-initialised
    (let [
           result (non-interactive-player1/player1 {:data ["get-first-move"]})
         ]
         {
           :status 200
           :headers {"Content-Type" "text/json"}
  ...
  (if @game-initialised
    (let [
	   move-value (:move-value (:params req))
           result (non-interactive-player1/player1 {:data ["get-next-move" move-value]})
         ]
         {
           :status 200
           :headers {"Content-Type" "text/json"}
...
(defn notify-move [req]
  ...
  (if @game-initialised
    (let [
	   move-value (:move-value (:params req))
           result (non-interactive-player1/player1 {:data ["notify-move" move-value]})
         ]
         {
           :status 200
           :headers {"Content-Type" "text/json"}
