(ns ikke-interaktiv-spiller1.spiller
  (:require [org.httpkit.server :as server]
	    ...
	    [games.draw-a-winner.ikke-interaktiv-spiller1 :as ikke-interaktiv-spiller1]
  )
  (:gen-class)
)
...
(defn initialiser-spil [req]
  (let [resultat (ikke-interaktiv-spiller1/spiller1 {:data ["initialiserSpil"]})]
...
(defn hent-foerste-traek [req]
  (if @spillet-er-initialiseret?
    (let [
           resultat (ikke-interaktiv-spiller1/spiller1 {:data ["hentFoersteTraek"]})
         ]
         {
           :status 200
           :headers {"Content-Type" "application/json"}
  ...
  (if @spillet-er-initialiseret?
    (let [
	   traekvaerdi (:traekvaerdi (:params req))
           resultat (ikke-interaktiv-spiller1/spiller1 {:data ["hentNaesteTraek" traekvaerdi]})
         ]
         {
           :status 200
           :headers {"Content-Type" "application/json"}
...
(defn meddel-traek [req]
  (if @spillet-er-initialiseret?
    (let [
	   traekvaerdi (:traekvaerdi (:params req))
           resultat (ikke-interaktiv-spiller1/spiller1 {:data ["meddelTraek" traekvaerdi]})
         ]
         {
           :status 200
           :headers {"Content-Type" "application/json"}
...
(defroutes app-routes
  (GET "/" [] "Dette er en ikke-interaktiv spiller1 til spillet \"Traek-en-vinder\".")
...
(defn -main
  ...
  [& args]
  (let [port (Integer/parseInt (or (System/getenv "PORT") "3201"))]
