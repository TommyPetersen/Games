...
(defn new-board-cell [x0 y0 x1 y1 margin-pct cell-frame-color chip-color]
  (let [
         ...
         cell-top (doto (new Polygon3D)
                           (.addPoint (new Point3D x0-d y0-d projection-plane-z (:top cell-frame-color)))
			   (.addPoint (new Point3D x1-d y0-d projection-plane-z (:top cell-frame-color)))
			   (.addPoint (new Point3D x1-d (- y0-d margin-y) projection-plane-z (:top cell-frame-color)))
			   (.addPoint (new Point3D x0-d (- y0-d margin-y) projection-plane-z (:top cell-frame-color)))
                   )
         cell-bottom (doto (new Polygon3D)
                           (.addPoint (new Point3D x0-d y1-d projection-plane-z (:bottom cell-frame-color)))
			   (.addPoint (new Point3D x1-d y1-d projection-plane-z (:bottom cell-frame-color)))
			   (.addPoint (new Point3D x1-d (+ y1-d margin-y) projection-plane-z (:bottom cell-frame-color)))
			   (.addPoint (new Point3D x0-d (+ y1-d margin-y) projection-plane-z (:bottom cell-frame-color)))
                     )
         cell-left (doto (new Polygon3D)
                           (.addPoint (new Point3D x0-d (- y0-d margin-y) projection-plane-z (:left cell-frame-color)))
			   (.addPoint (new Point3D (+ x0-d margin-x) (- y0-d margin-y) projection-plane-z (:left cell-frame-color)))
			   (.addPoint (new Point3D (+ x0-d margin-x) (+ y1-d margin-y) projection-plane-z (:left cell-frame-color)))
			   (.addPoint (new Point3D x0-d (+ y1-d margin-y) projection-plane-z (:left cell-frame-color)))
                     )
         cell-right (doto (new Polygon3D)
                           (.addPoint (new Point3D (- x1-d margin-x) (- y0-d margin-y) projection-plane-z (:right cell-frame-color)))
			   (.addPoint (new Point3D x1-d (- y0-d margin-y) projection-plane-z (:right cell-frame-color)))
			   (.addPoint (new Point3D x1-d (+ y1-d margin-y) projection-plane-z (:right cell-frame-color)))
			   (.addPoint (new Point3D (- x1-d margin-x) (+ y1-d margin-y) projection-plane-z (:right cell-frame-color)))
                     )
	 cell-chip (doto (new Polygon3D)
                           (.addPoint (new Point3D (+ x0-d margin-x) (- y0-d margin-y) projection-plane-z chip-color))
			   (.addPoint (new Point3D (- x1-d margin-x) (- y0-d margin-y) projection-plane-z chip-color))
			   (.addPoint (new Point3D (- x1-d margin-x) (+ y1-d margin-y) projection-plane-z chip-color))
			   (.addPoint (new Point3D (+ x0-d margin-x) (+ y1-d margin-y) projection-plane-z chip-color))
                   )
       ]
