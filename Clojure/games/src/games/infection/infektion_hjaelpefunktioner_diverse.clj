(ns games.infection.infektion-hjaelpefunktioner-diverse
...
(defn valid-move-seq [board symbol]
  (let [board-move-seq (for [
                              fra-koordinat (get-all-coords-with-given-symbol board symbol)
	                      til-koordinat (get-neighbourhood board fra-koordinat 2)
	                      :when (= (game-utils-misc/lookup board til-koordinat) nil)
                            ]
                            {
                              :fra-koordinat fra-koordinat
	                      :til-koordinat til-koordinat
                            }
		       )
       ]
       (if (> (count board-move-seq ) 0)
           board-move-seq
	   (seq [{:fra-koordinat [-1 -1] :til-koordinat [-1 -1]}])
       )
  )
)
...
(defn make-move [board move]
  (if (= move {:fra-koordinat [-1 -1] :til-koordinat [-1 -1]})
      board
      (let [
             fra-koordinat (:fra-koordinat move)
	     til-koordinat (:til-koordinat move)
	     symbol (game-utils-misc/lookup board fra-koordinat)
	     is-spreading? (and (< (Math/abs (- (first fra-koordinat) (first til-koordinat))) 2)
	                        (< (Math/abs (- (second fra-koordinat) (second til-koordinat))) 2))
	     cell-updates (if is-spreading? [{:coord til-koordinat :symbol symbol}]
	                                    [{:coord fra-koordinat :symbol nil} {:coord til-koordinat :symbol symbol}])
	     neighbourhood-infected-by-others (get-neighbourhood-infected-by-others board til-koordinat 1)
...
(defn cannot-move? [board symbol]
  (let [valid-move-seq (valid-move-seq board symbol)]
       (and (= (count valid-move-seq) 1) (= (first valid-move-seq) {:fra-koordinat [-1 -1] :til-koordinat [-1 -1]}))
  )
)
...
(defn max-val [board alpha beta ply-depth max-ply-depth]
  (if (cutoff? board ply-depth max-ply-depth)
    ...
    (let [
	   ...
	   m (atom {:fra-koordinat [0 0] :til-koordinat [0 0]})
